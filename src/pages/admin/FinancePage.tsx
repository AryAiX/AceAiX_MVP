...
import {
  ...
  Download, CheckCircle, Clock, XCircle, RefreshCw, Filter,
...
function Chip({ label }: { label: string }) {
  ...
  return (
    <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase" style={s}>{label.replace('_', ' ')}</span>
  );
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: '#9DB0C6' }}>
            Revenue, subscriptions, partner payouts, refunds
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'MRR',           value: '$70.4k',  delta: '+13.2%', color: '#B8F135', icon: DollarSign,   up: true  },
          { label: 'ARR',           value: '$845k',   delta: '+18.4%', color: '#2F80ED', icon: TrendingUp,   up: true  },
          { label: 'Pending Payouts', value: '$9,800', delta: '3 partners', color: '#F5A623', icon: CreditCard, up: false },
          { label: 'Refund Rate',   value: '0.8%',    delta: '-0.2%',  color: '#1FB57A', icon: RefreshCw,    up: true  },
        ].map(s => (
      ...
      {tab === 'overview' && (
        <div className="space-y-5">
            ...
            <div className="rounded-2xl p-5" style={{ background: '#0F1E32', border: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <AreaChart data={MRR_DATA} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="mrr" stroke="#B8F135" strokeWidth={2} fill="url(#gVolt)" name="MRR" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            <div className="rounded-2xl p-5" style={{ background: '#0F1E32', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            {[
              { label: 'New MRR (Jun)', value: '$9,800',  color: '#1FB57A' },
              { label: 'Churned MRR',  value: '$1,400',  color: '#EF5350' },
              { label: 'Net New MRR',  value: '+$8,400', color: '#B8F135' },
            ].map(m => (
              <div key={m.label} className="rounded-xl p-4 text-center"
                style={{ background: '#0F1E32', border: '1px solid rgba(255,255,255,0.07)' }}>
                <p className="text-xs mb-1" style={{ color: '#9DB0C6' }}>{m.label}</p>
      ...
      {tab === 'subscriptions' && (
        <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
          <div className="grid px-4 py-3 text-[10px] font-bold uppercase tracking-widest"
            style={{ background: '#0A1828', color: '#9DB0C6', gridTemplateColumns: '1fr 120px 80px 80px 90px 80px' }}>
            <span>Organization</span><span>Plan</span><span>Amount</span><span>Status</span><span>Next Billing</span><span>Seats</span>
          </div>
          {SUBSCRIPTIONS.map((s, i) => (
            <div key={s.id} className="grid px-4 py-3 items-center text-xs"
              style={{
                gridTemplateColumns: '1fr 120px 80px 80px 90px 80px',
                background: i % 2 === 0 ? '#0D1A2B' : 'rgba(255,255,255,0.015)',
                borderTop: '1px solid rgba(255,255,255,0.04)',
              }}>
              <span className="text-white font-medium">{s.org}</span>
              <span style={{ color: '#9DB0C6' }}>{s.type}</span>
              <span style={{ color: '#B8F135' }}>${s.amount}/mo</span>
              <Chip label={s.status} />
      ...
      {tab === 'payouts' && (
        <div className="space-y-4">
          <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
            <div className="grid px-4 py-3 text-[10px] font-bold uppercase tracking-widest"
              style={{ background: '#0A1828', color: '#9DB0C6', gridTemplateColumns: '1fr 100px 80px 80px 90px 120px' }}>
              <span>Partner</span><span>Type</span><span>Amount</span><span>Status</span><span>Due</span><span>Actions</span>
            </div>
            {PAYOUTS.map((p, i) => (
              <div key={p.id} className="grid px-4 py-3 items-center text-xs"
                style={{
                  gridTemplateColumns: '1fr 100px 80px 80px 90px 120px',
                  background: i % 2 === 0 ? '#0D1A2B' : 'rgba(255,255,255,0.015)',
                  borderTop: '1px solid rgba(255,255,255,0.04)',
                }}>
                <span className="text-white font-medium">{p.partner}</span>
                <span style={{ color: '#9DB0C6' }}>{p.type}</span>
                <span style={{ color: '#B8F135' }}>${p.amount.toLocaleString()}</span>
                <Chip label={p.status} />
      ...
      {tab === 'refunds' && (
        <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
          <div className="grid px-4 py-3 text-[10px] font-bold uppercase tracking-widest"
            style={{ background: '#0A1828', color: '#9DB0C6', gridTemplateColumns: '1fr 120px 80px 90px 1fr 100px' }}>
          ...
          {REFUNDS.map((r, i) => (
            <div key={r.id} className="grid px-4 py-3 items-center text-xs"
              style={{
                gridTemplateColumns: '1fr 120px 80px 90px 1fr 100px',
                background: i % 2 === 0 ? '#0D1A2B' : 'rgba(255,255,255,0.015)',
                borderTop: '1px solid rgba(255,255,255,0.04)',
              }}>
              <span className="text-white font-medium">{r.user}</span>
              <span style={{ color: '#9DB0C6' }}>{r.plan}</span>
              <span style={{ color: '#EF5350' }}>-${r.amount}</span>
